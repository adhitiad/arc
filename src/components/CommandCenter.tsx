...
export function CommandCenter() {
  ...
  const handleSelect = (value: string) => {
    ...
    const orderRegex = /^(buy|sell)\s+([a-zA-Z0-9./=-]{3,})\s+(\d+)$/i;
        ...
        <CommandGroup heading="Smart Actions">
          {liveSignals.length === 0 && (
            <CommandItem onSelect={() => handleSelect("battle")}>
              ...
              <span>No active signals yet</span>
            </CommandItem>
          )}
          {liveSignals.map((signal: any) => (
            <CommandItem
              key={`${signal.symbol}-${signal.action}`}
              onSelect={() => handleSelect(`${signal.action} ${signal.symbol} 1`)}
            >
              <Zap className="mr-2 h-4 w-4 text-yellow-500" />
              <span>
                {signal.action} {signal.symbol}
                {signal.prob ? ` (${signal.prob})` : ""}
              </span>
            </CommandItem>
          ))}
           <CommandItem onSelect={() => handleSelect("battle")}>
